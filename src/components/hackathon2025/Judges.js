...
export default function Judges({headerStyle}) {
    return (
        <section id="judges" class="pt-6 pb-12 items-center bg-gray-900 text-white lg:min-h-screen">
            <h1 className={`text-5xl font-bold text-center m-4 mb-8 ${headerStyle}`}>Hackathon Judges</h1>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 lg:w-2/3 mx-auto justify-center gap-4 m-4">
                {judges.map((officer) => (
                    <a href={officer.url ? officer.url : "https://acm.sdsu.edu/hack"} target="_blank">
                        <div key={officer.id} className="min-h-64 min-w-16 text-center text-gray-200 bg-gradient-to-r from-acm-dark-blue to-acm-dark-blue via-acm-blue p-4 rounded-lg flex flex-col items-center justify-center">
