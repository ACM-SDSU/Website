...
export default function Prizes({headerStyle}) {
    return (
        <section id="prizes" class="pt-6 pb-12 items-center bg-gray-900 text-white lg:min-h-screen">
            ...
            <p class="w-4/5 lg:w-2/3 mx-auto text-center">Participants who develop strong, compelling projects, are eligible to receive the following prizes.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 lg:w-2/3 mx-auto justify-center gap-4 m-4">
                {prizes.map((prize) => (
                    <div key={prize.id} className="min-h-32 min-w-16 text-center text-gray-200 bg-gradient-to-r from-acm-orange to-acm-orange via-acm-yellow p-2 rounded-2xl flex flex-col items-center justify-center">
                        <img src={prize.photo} class="w-28 h-28 mb-1 rounded-xl"></img>
